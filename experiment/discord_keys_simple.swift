  ...
  for action in actions {
    if let key = action.key {
      ...
      if let code = SPECIAL_KEYS[keyLower] {
        sendKeyToPid(code, pid, modifiers: action.modifiers ?? [])
      } else if let char = keyLower.first, let keyCode = KEY_CODES[char] {
        sendKeyToPid(keyCode, pid, modifiers: action.modifiers ?? [])
      }
    }
